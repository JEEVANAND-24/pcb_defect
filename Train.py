...
def DenseNet(input_shape=None, dense_blocks=3, dense_layers=-1, growth_rate=12, nb_classes=None, dropout_rate=None,
    ...
    if nb_classes is None:
        raise Exception('Please define the number of classes (e.g., num_classes=10). This is required for the final softmax.')

    if compression <= 0.0 or compression > 1.0:
        raise Exception('Compression must be a value between 0.0 and 1.0. If you set compression to 1.0, it will be turned off.')

    if type(dense_layers) is list:
        if len(dense_layers) != dense_blocks:
            raise AssertionError('Number of dense blocks must be the same length as specified layers.')
    elif dense_layers == -1:
        if bottleneck:
...
if __name__ == '__main__':
    model, model_name = DenseNet(input_shape=(64, 64, 1), dense_blocks=2, dense_layers=6, growth_rate=32, nb_classes=11,
                             bottleneck=True, depth=27, weight_decay=1e-5)
    opt = SGD(learning_rate=0.01, momentum=0.9)
    model.compile(optimizer=opt, loss='categorical_crossentropy', metrics=['accuracy'])
    train_datagen = ImageDataGenerator(data_format="channels_last")
    train_generator = train_datagen.flow_from_directory('C:\\Users\\rjeev\\Desktop\\project\\data set\\DeepPCB-master\\PCBData',
                                                        target_size=(64, 64), color_mode='grayscale', batch_size=8)
    STEP_SIZE_TRAIN = train_generator.n // train_generator.batch_size
    lrate = LearningRateScheduler(step_decay, verbose=1)
    callbacks_list = [lrate]
    model.fit(train_generator, steps_per_epoch=STEP_SIZE_TRAIN, epochs=15, callbacks=callbacks_list, verbose=1)
    from tensorflow.keras.models import save_model
    save_model(model, 'C:\\Users\\rjeev\\Desktop\\project\\data set\\PCB-Defects-Classification-Using-Deep-Learning-main\\Code\\pcb_defect.h5')
